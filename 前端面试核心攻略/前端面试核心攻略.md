...
绝对定位保持上下左右各50%的距离不能达到居中的效果，是因为绝对定位的基准线是子元素的左边缘和顶边缘，但我们是希望子元素的中心和容器的边缘保持50%的距离，因此在元素宽高都已知的情况下，通过margin的负值来中和边缘和中心的距离。

- margin：auto

对想要水平居中的某个元素，我们设置```margin: 0 auto;```可以达到想要的效果。auto属性在任何情况下，只会取下面两种值的一个：

1.父元素剩余空间的宽度

2.0

当元素的布局方式为static/relative且宽高已知，auto取1的值，当元素的布局方式为position/absolute/fixed或者float/inline或者宽高未知，auto就取2的值。

但auto只针对水平方向生效，垂直方向不会自动填充。

利用元素的流体特性，我们也可以实现元素的垂直居中。当一个绝对定位元素，其对立定位方向属性同时有具体定位数值的时候，流体特性就发生了。流体特性使得元素可以自动填充父级元素的可用尺寸。
...
在宽高不定的情况下，一般用transform这个属性来达到目的：
...
transform是css3引入的一个动画属性，允许我们对元素进行旋转、缩放、移动和倾斜。

translate对应的是元素的移动能力，接受的两个参数分别是X轴和Y轴的移动量，这里的50%意味着元素需要横向纵向分别移动自身宽高的50%。
...
Table-cell布局不要求子元素定高，但是父元素的宽高如果用百分比的写法会有问题，所以需要给父元素确定的宽高。
...
原因是css中的1px与移动设备上的1px不相等，他们之间的比例计算方式```window.devicePixelRatio = 设备的物理像素 / css像素 ```

**1px问题的解决方案**

- 直接写比例计算后的结果

先在JS中拿到window.devicePixelRatio的值，然后把这个值传给CSS的data里。

```jsx
  <div id="container" data-device={{window.devicePixelRatio}}></div>
```

然后就可以在CSS中用属性选择器来命中devicePixelRatio为某一个值的情况。

```css
.father[data-device="2"] {
...
我们的思路是在目标元素的后面追加一个::after伪元素，让这个元素的布局为absolute之后、整个展开铺在目标元素上，然后把它的宽和高都设置为目标元素的两倍，border值设为 1px。接着借助 CSS 动画特效中的放缩能力，把整个伪元素缩小为原来的 50%。此时，伪元素的宽高刚好可以和原有的目标元素对齐，而 border 也缩小为了 1px 的二分之一，间接地实现了 0.5px 的效果。
...
当子元素浮动后，脱离了文档流，会导致父元素的宽高丢失。

![浮动影响](./float-influence.jpeg)

如果我们用BFC来清除浮动带给父元素的影响，比如给父元素加上属性```overflow:hidden;``` 就能在实现子元素float的布局效果的情况下，避免了对父元素的影响。因为BFC是独立作用域，在把父元素创建为一个BFC后，为了保证子元素不会影响外界或被外界影响，会自动把宽高适应到能囊括子元素。
...
![margin overlapping](./margin-overlapping.jpeg)

在CSS中，相邻的两个盒子的外边距会被结合成一个单独的外边距，这种合并外边距的方式叫做折叠。

如果要清除折叠效果，就给两个盒子各创建一个父元素，让父元素是一个BFC。
...
![清除折叠](./clear-overlapping.jpeg)

此时，不见兄弟元素之间的外边距生效了，子元素和父元素之间的外边距也生效了。通过把两个子元素放进两个不同的BFC里，利用它的独立特性，可以是两个子元素都避免与外界产生重叠关系。
...
<body>
   ...
   <div class="box2">我没有被浮动，我是修言的两列布局测试小弟我没有被浮动，我是修言的两列布局测试小弟我没有被浮动，我是修言的两列布局测试小弟我没有被浮动，我是修言的两列布局测试小弟我没有被浮动，我是修言的两列布局测试小弟</div>
</body>
```

![自适应布局](./display-adaptive.jpeg)

虽然实现了自适应布局，但脱离了文档流的A元素会遮挡B元素，因此要想解决覆盖问题，我们为B元素创建一个BFC。

![清除覆盖](./clear-cover.jpeg)

#### CSS工程化

**预处理器**

预处理器就是CSS世界的轮子，它支持我们写类CSS代码并把它编译成CSS代码。

传统的CSS在应用中有这几点不能满足使用诉求：

- 我们希望能优化 CSS 文件的目录结构，对现有的 CSS 文件实现复用；
- 我们希望能写出结构清晰、简明易懂的 CSS，需要它具有一目了然的嵌套层级关系；希望它具有变量特征、计算能力、循环能力等等更强的可编程性，这样我们可以少写一些无用的代码；
- 具有更强的可维护性

而五花八门的预处理器普遍会具有这样的特性：

- 嵌套代码的能力，通过嵌套来反映不同 css 属性之间的层级关系 ；
- 支持定义CSS变量
- 提供计算函数
- 允许对代码片段进行extend和minxin
- 支持循环语句
- 支持CSS文件模块化，实现复用

常见的预处理器Sass和Less：[less 文档](http://lesscss.cn/)、[sass 文档](https://www.sass.hk/)
